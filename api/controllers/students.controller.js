...
//     "b4651d6514a046118f21e611076f5e1a",
//     false
//   );
//   try {
//     var geoResult = {};
...
//                 //info device of the user who try to login
//                 const infodevice = {
//                   ip: geoResult.ip,
//                   city: geoResult.city,
...
//                   country_flag: geoResult.country_flag,
...
userController.googleOAuth = async (req, res, next) => {
  ...
  try {
    ...
    await ns8ds.score(options, async function(err, result) {
      if (!err && result) {
        const foundUser = await User.findOne({ "google.id": user.google.id });
        if (foundUser) {
          // Generate token
...
var listStudents=[]
    ...
    //let arrOfAccount = [foundUser]; //because in usersReducer we have users:[] as state
    if (foundUser) {
      listStudents.push({student:foundUser,active:el.active})
    } 
    }
    if (listStudents.length > 0) {
      res.send({ users:listStudents });
    } else {
